              ...
              <div className="file-list">
                {files.map((file, index) => (
                  <div key={`${file.name}_${index}`} className="file-item">
                    ...
                    <button className="file-remove" onClick={(event) => { event.stopPropagation(); removeFile(index); }}>✕</button>
                  </div>
                ))}
              </div>
            )}
              ...
              <div className="file-list">
                {docxFiles.map((file, index) => (
                  <div key={`${file.name}_${index}`} className="file-item">
                    ...
                    <span className="file-name">{file.name}</span>
                    <span className="file-size">{(file.size / 1024 / 1024).toFixed(1)} МБ</span>
                    <button className="file-remove" onClick={(event) => { event.stopPropagation(); removeDocxFile(index); }}>✕</button>
                  </div>
                ))}
              </div>
            )}
