...
- social_id — идентификатор пользователя в мессенджере или соцсети (числовой ID, username)
...
async function callClaude(messages, apiKey, system) {
  const body = { model: PROVIDERS.claude.model, max_tokens: 32000, messages };
...
async function callOpenAI(messages, apiKey, system) {
  ...
  const resp = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({ model: PROVIDERS.openai.model, max_tokens: 32000, messages: oaiMessages }),
  });
  if (!resp.ok) {
    const err = await resp.json().catch(() => ({}));
