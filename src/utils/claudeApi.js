...
Адреса — самый частый тип ПД в юридических документах. Находи ВСЕ адреса, включая:
...
Если у одного человека два адреса (регистрации и проживания) — это ДВЕ отдельные записи в otherPD.
Адрес в value — это полная строка начиная от названия региона/города до квартиры.
Пример: "зарегистрированный по адресу: Самарская область, Приволжский район, с. Общаровка, ул. Школьная, д. 20, кв. 5 и фактически проживающего по адресу: г. Самара, ул. Рабочая, д. 16, кв. 10"
→ запись 1: value="Самарская область, Приволжский район, с. Общаровка, ул. Школьная, д. 20, кв. 5", replacement="[адрес]"
→ запись 2: value="г. Самара, ул. Рабочая, д. 16, кв. 10", replacement="[адрес]"
...
export async function recognizeDocument(images, apiKey, provider, onProgress) {
  ...
  const cleanedTexts = texts.map(t => {
    // Сначала склеиваем переносы слов через дефис (Са-\nмарского → Самарского)
    ...
    const dehyphenated = t
      .replace(/([А-яЁёa-zA-Z])-\r?\n([а-яёa-z])/g, '$1$2')  // перенос через \n
      .replace(/([А-яЁёa-zA-Z])- ([а-яёa-z])/g, '$1$2');       // перенос через пробел
