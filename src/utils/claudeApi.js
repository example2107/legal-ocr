...
При OCR часто возникают ошибки: система может заменить неразборчивое слово на похожее существующее слово, но с другим смыслом. Например: "сувоей" -> "судьёй", "зорегистрировано" -> "зарегистрировано".

Твоя задача: найти места в тексте, где слово выглядит семантически неуместным в контексте предложения — как будто OCR ошибся и подставил не то слово.
...
- Отмечай только слова, которые явно не вписываются в контекст
...
async function analyzePD(fullText, apiKey, onProgress) {
  ...
  // OCR quality check — find semantically suspicious words
